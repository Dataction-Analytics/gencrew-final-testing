...
**`gencrew-final-testing-dbt/`** — the dbt project, the stack chosen for
this delivery.

```bash
cd gencrew-final-testing-dbt
...
Details per table: `gencrew-final-testing-dbt/INCREMENTAL.md`.
...
Formulas and grains: `gencrew-final-testing-dbt/KPI_CATALOG.md`.

## 6. Business rules

13 of 13 mapping-sheet rules were
applied automatically; the remainder are listed with reasons in
`gencrew-final-testing-dbt/BUSINESS_RULES.md` for a human to resolve.
